...
def test_db_connection():
    ...
    try:
        ...
        # Intentar conexión básica con SSL forzado
        import urllib.parse
        parsed = urllib.parse.urlparse(database_url)
        
        print(f"DEBUG_DB_INIT: Intentando conexión a {parsed.hostname}:{parsed.port}/{parsed.path[1:]}")
        ...
        print(f"DEBUG_DB_INIT: Conexión exitosa - PostgreSQL {version[0]}")
        return True
        ...
        if getattr(e, 'pgcode', None) == '08001':
            print("DEBUG_DB_INIT_DIAGNÓSTICO: Error de red/firewall/SSL (08001)")
        elif getattr(e, 'pgcode', None) == '28P01':
            print("DEBUG_DB_INIT_DIAGNÓSTICO: Error de autenticación/contraseña (28P01)")
        else:
            print(f"DEBUG_DB_INIT_DIAGNÓSTICO: Error no identificado: {getattr(e, 'pgcode', 'N/A')}")
...
class DatabaseManager:
    ...
    def __init__(self):
        ...
        self._load_config()
        self._check_connection()  # Verificar conexión al inicio
    ...
    def _load_config(self):
        ...
        try:
            ...
            # Forzar SSL para producción y desarrollo
            self.config['sslmode'] = 'require'  # Requerido para Render y nube
            print("DEBUG_DB: SSL requerido para conexión segura")
    ...
    def _check_connection(self):
        """Verificar conexión a base de datos al inicio con 3 intentos"""
        import time
        ...
        for intento in range(1, 4):
            try:
                ...
                if intento < 3:
                    ...
                    raise Exception(f"Error verificando conexión a base de datos después de 3 intentos: {e}")
    ...
    def get_connection(_self):
        ...
        try:
            ...
            # Crear nueva conexión con SSL obligatorio para Render
            connection_params = {
                ...
                'sslmode': 'require'  # Obligatorio para Render y nube
            }
            
            print(f"DEBUG_DB: Conectando a {_self.config['host']}:{_self.config['port']}/{_self.config['database']} con SSL")
            
            try:
                _self._connection = psycopg2.connect(**connection_params)
                print(f"DEBUG_DB: Conexión establecida exitosamente")
            except OperationalError as e:
...
def authenticate_user(username, password):
    ...
    try:
        ...
        # Buscar usuario por cédula primero (prioridad para login)
        debug_query = """
        SELECT u.cedula_usuario, u.login_usuario, u.rol, u.activo, u.contrasena
        FROM usuarios u
        ...
        debug_result = execute_query(debug_query, (normalized_username,), fetch_one=True)
        
        if debug_result:
            logger.info(f"DEBUG: Usuario encontrado por cédula:")
            ...
            # Si no encuentra por cédula, buscar por login_usuario (fallback)
            fallback_query = """
            SELECT u.cedula_usuario, u.login_usuario, u.rol, u.activo, u.contrasena
            FROM usuarios u
            WHERE u.login_usuario = %s AND u.activo = TRUE
            """
            fallback_result = execute_query(fallback_query, (cleaned_username,), fetch_one=True)
            
            if fallback_result:
                logger.info(f"DEBUG: Usuario encontrado por login_usuario:")
                logger.info(f"  - cedula_usuario: '{fallback_result['cedula_usuario']}'")
                logger.info(f"  - login_usuario: '{fallback_result['login_usuario']}'")
                logger.info(f"  - contrasena (hash): '{fallback_result['contrasena']}'")
                logger.info(f"  - contrasena coincide: {fallback_result['contrasena'] == hashed_password}")
                debug_result = fallback_result
            else:
                logger.warning(f"DEBUG: Usuario NO encontrado en BD con cédula: '{normalized_username}' ni login: '{cleaned_username}'")
        
        # Ejecutar consulta completa con verificación de contraseña
        try:
            # Priorizar búsqueda por cédula
            if debug_result and debug_result['cedula_usuario'] == normalized_username:
                result_query = """
                SELECT u.cedula_usuario, u.login_usuario, u.rol, u.activo, u.contrasena,
                       p.nombre, p.apellido, p.telefono, p.direccion
                FROM usuarios u
                LEFT JOIN persona p ON u.cedula_usuario = p.cedula
                WHERE u.cedula_usuario = %s AND u.contrasena = %s AND u.activo = TRUE
                """
                result = execute_query(result_query, (normalized_username, hashed_password), fetch_one=True)
            else:
                # Fallback a búsqueda por login_usuario
                result_query = """
                SELECT u.cedula_usuario, u.login_usuario, u.rol, u.activo, u.contrasena,
                       p.nombre, p.apellido, p.telefono, p.direccion
                FROM usuarios u
                LEFT JOIN persona p ON u.cedula_usuario = p.cedula
                WHERE u.login_usuario = %s AND u.contrasena = %s AND u.activo = TRUE
                """
                result = execute_query(result_query, (cleaned_username, hashed_password), fetch_one=True)
