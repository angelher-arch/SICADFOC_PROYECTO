...
class DatabaseManager:
    ...
    def _check_connection(self):
        ...
        try:
            ...
            print(f"DEBUG_DB: Conexión exitosa - PostgreSQL {version[0]}")
            
        except OperationalError as e:
            print(f"DEBUG_DB_ERROR: Error de conexión a PostgreSQL: {e}")
            print(f"DEBUG_DB_ERROR: Host: {self.config.get('host', 'unknown')}")
            print(f"DEBUG_DB_ERROR: Port: {self.config.get('port', 'unknown')}")
            print(f"DEBUG_DB_ERROR: Database: {self.config.get('database', 'unknown')}")
            print(f"DEBUG_DB_ERROR: User: {self.config.get('user', 'unknown')}")
            raise Exception(f"No se puede conectar a la base de datos: {e}")
        except Exception as e:
            print(f"DEBUG_DB_ERROR: Error inesperado en conexión: {e}")
            raise Exception(f"Error verificando conexión a base de datos: {e}")
