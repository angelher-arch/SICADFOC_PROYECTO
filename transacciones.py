...
class TransaccionFOC26:
        ...
        try:
            ...
            with self.db.cursor() as cursor:
                ...
                """, (
                    ...
                    datos_profesor['email'],
                    cedula,
                    datos_profesor.get('rol', 'Profesor'),
                    True
                ))
                id_usuario = cursor.fetchone()['id']
    ...
    def obtener_configuraciones(self, claves=None):
        """Obtener valores de configuración del sistema."""
        try:
            with self.db.cursor() as cursor:
                query = "SELECT clave, valor, descripcion FROM configuracion"
                params = []
                if claves:
                    query += " WHERE clave = ANY(%s)"
                    params.append(claves)
                query += " ORDER BY clave"
                cursor.execute(query, params)
                filas = cursor.fetchall()
                return {fila['clave']: {'valor': fila.get('valor'), 'descripcion': fila.get('descripcion')} for fila in filas}
        except Exception as e:
            return {}

    def actualizar_configuraciones_transaccional(self, cambios, id_usuario=None):
        """Actualizar o insertar configuraciones con control transaccional."""
        try:
            with self.db.cursor() as cursor:
                id_transaccion = self.generar_id_transaccion()
                cursor.execute("BEGIN")
                ...
                for clave, valor, descripcion in cambios:
                    cursor.execute("SELECT id_config FROM configuracion WHERE clave = %s", (clave,))
                    registro = cursor.fetchone()
                    if registro:
                        cursor.execute(
                            "UPDATE configuracion SET valor = %s, descripcion = %s, actualizado_en = CURRENT_TIMESTAMP WHERE clave = %s",
                ...
                cursor.execute(
                    "INSERT INTO auditoria (id_transaccion, tipo_transaccion, descripcion, id_usuario, detalles, fecha_hora) VALUES (%s, %s, %s, %s, %s, %s)",
                    (
                        id_transaccion,
                        'ACTUALIZAR_CONFIGURACION',
                        'Actualización de parámetros de configuración del sistema',
                        id_usuario,
