...
class TransaccionFOC26:
        ...
        try:
            ...
            with self.db.cursor() as cursor:
                ...
                """, (
                    id_persona,
                    id_usuario,
                    datos_estudiante['codigo_estudiantil'],
                    datos_estudiante['carrera'],
                    datos_estudiante['semestre_actual'],
            ...
            with self.db.cursor() as cursor:
                query = """
                    SELECT p.id, p.nombre, p.apellido, p.cedula, p.email, p.telefono,
                           u.login_usuario, e.codigo_estudiantil, e.carrera, e.semestre_actual, e.estado
                    FROM persona p
                ...
                if carrera:
                    query += " AND e.carrera ILIKE %s"
                    params.append(f"%{carrera}%")
                if semestre:
                    query += " AND e.semestre_actual ILIKE %s"
    ...
    def obtener_profesor_por_cedula(self, cedula):
        ...
        try:
            ...
            with self.db.cursor() as cursor:
                cursor.execute("""
                    SELECT pr.*, p.nombre, p.apellido, p.cedula, p.email as email_personal, u.email as email_institucional, u.id_usuario
                    FROM profesor pr
                    JOIN persona p ON pr.id_persona = p.id
                    JOIN usuario u ON pr.id_usuario = u.id_usuario
    ...
    def obtener_estudiante_por_cedula(self, cedula):
        ...
        try:
            ...
            with self.db.cursor() as cursor:
                cursor.execute("""
                    SELECT e.*, p.nombre, p.apellido, p.cedula, p.email as email_personal, u.email as email_institucional, u.id_usuario
                    FROM estudiante e
    ...
    def historial_estudiantil_por_cedula(self, cedula):
        ...
        try:
            with self.db.cursor() as cursor:
                query = """
                    SELECT p.cedula, p.nombre, p.apellido, e.codigo_estudiantil, e.carrera, e.semestre_actual,
