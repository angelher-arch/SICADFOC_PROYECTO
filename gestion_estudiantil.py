            ...
            if st.session_state.get('_buscar_estudiantes', False) or any([search_cedula, search_nombre, search_carrera, search_semestre]):
                ...
                if estudiantes:
                    ...
                    with st.expander("Editar perfil del estudiante"):
                        ...
                        with col2:
                            carrera_actual = st.selectbox("Carrera", get_carreras(), index=get_carreras().index(seleccionado['carrera']) if seleccionado['carrera'] in get_carreras() else 0) if seleccionado['carrera'] else st.selectbox("Carrera", get_carreras())
                            semestre_actual = st.selectbox("Semestre", get_semestres(), index=get_semestres().index(seleccionado['semestre_actual']) if seleccionado['semestre_actual'] in get_semestres() else 0) if seleccionado['semestre_actual'] else st.selectbox("Semestre", get_semestres())
                            estado_actual = st.selectbox("Estado", get_estados_registro(), index=get_estados_registro().index(seleccionado['estado']) if seleccionado['estado'] in get_estados_registro() else 0)
                        ...
                        if st.button("Actualizar Estudiante"):
                            ...
                            db.ejecutar_consulta(
                                "UPDATE estudiante SET carrera = %s, semestre_actual = %s, estado = %s WHERE id_persona = %s",
                                (carrera_actual, semestre_actual, estado_actual, seleccionado['id'])
                            )
