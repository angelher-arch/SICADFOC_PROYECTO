...
class ConexionPostgreSQL:
    ...
    def conectar(self):
        """Establecer conexión usando DATABASE_URL con fallback local"""
        try:
            if self.connection and not self.connection.closed:
                return self.connection
            
            # PRIORIDAD 1: Usar DATABASE_URL si está disponible
            database_url = os.environ.get('DATABASE_URL', 'postgresql://postgres:password@localhost:5432/foc26db')
            
            if database_url and database_url.strip() and not database_url.startswith('postgresql://postgres:password@localhost:5432/foc26db'):
                ...
                try:
                    ...
                    logger.info("Conexión Railway establecida exitosamente")
                    return self.connection
                except Exception as e:
                    logger.warning(f"Error con DATABASE_URL: {e}, usando fallback local")
            
            # FALLBACK: Conexión local
            logger.info(f"Conectando a PostgreSQL local en {self.host}:{self.port}/{self.database}")
