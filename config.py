...
class ConfigManager:
    ...
    def __init__(self):
        ...
        # Solo cargar .env si existe (evita errores en producción)
        try:
            load_dotenv()
            print("DEBUG_CONFIG: .env cargado si existe")
        except Exception as e:
            print(f"DEBUG_CONFIG: Error cargando .env (ignorado en producción) - {e}")
        
        self._config = self._load_environment_config()
        print(f"DEBUG_CONFIG: Configuración ambiente cargada")
    ...
    def _get_production_config(self) -> Dict[str, Any]:
        ...
        print(f"DEBUG_CONFIG: DATABASE_URL encontrada: {'SÍ' if database_url else 'NO'}")
        
        if not database_url:
            print("DEBUG_CONFIG_ERROR: DATABASE_URL no encontrada en producción")
            print("DEBUG_CONFIG_ERROR: Variables de entorno disponibles:")
            for key, value in os.environ.items():
                if 'DATABASE' in key or 'DB' in key:
                    print(f"  {key}: {'***' if 'PASSWORD' in key else value}")
            raise ValueError("Variable DATABASE_URL no encontrada en el entorno")
        
        # Parsear DATABASE_URL con parámetros SSL
        try:
            parsed = urllib.parse.urlparse(database_url)
            print(f"DEBUG_CONFIG: DATABASE_URL parseada - Host: {parsed.hostname}")
            
            # Extraer parámetros de consulta (como sslmode)
            query_params = urllib.parse.parse_qs(parsed.query)
            ssl_mode = query_params.get('sslmode', ['require'])[0]
            print(f"DEBUG_CONFIG: SSL mode detectado: {ssl_mode}")
            
        except Exception as e:
            print(f"DEBUG_CONFIG_ERROR: Error parseando DATABASE_URL - {e}")
            raise
        ...
        return {
            ...
            'database': {
                ...
                'sslmode': ssl_mode,  # Forzar SSL para Render
            },
            'pool': {
                'min_connections': 2,
                'max_connections': 10,
