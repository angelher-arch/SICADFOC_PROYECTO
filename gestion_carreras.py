...
try:
    ...
    from servicio_unificado_optimizado import ejecutar_query, limpiar_conexiones, obtener_columna_cedula
except ImportError as e:
    st.error(f"Error importando módulos: {e}")
    sys.exit(1)

def obtener_listado_carreras():
    """Función unificada para obtener listado de carreras desde la base de datos"""
    try:
        query_carreras = """
        SELECT id_carrera, nombre_carrera, codigo_carrera, activo, fecha_creacion
        FROM carrera
        ORDER BY nombre_carrera
        """
        
        resultado = ejecutar_query(query_carreras, fetch_all=True)
        ...
        # No mostrar error, solo registrar en log si estuviera disponible
        # st.error(f"Error técnico en consulta de carreras: {error}")
        return []

def editar_carrera(id_carrera, nombre_carrera, codigo_carrera, activo):
    """Función para editar una carrera existente"""
    try:
        query_update = """
        UPDATE carrera 
        SET nombre_carrera = %s, codigo_carrera = %s, activo = %s
        WHERE id_carrera = %s
        """
        
        resultado = ejecutar_query(query_update, params=(nombre_carrera, codigo_carrera, activo, id_carrera))
        
        return resultado is not None
        
    except Exception as error:
        st.error(f"Error editando carrera: {error}")
        return False
    finally:
        try:
            limpiar_conexiones()
...
class GestionCarreras:
    ...
    def mostrar_listado_carreras(self):
        ...
        try:
            # Obtener carreras usando la función global
            carreras = obtener_listado_carreras()
            
            if not carreras:
                st.info("No hay carreras disponibles en el sistema.")
                return
    ...
    def mostrar_formulario_edicion(self, id_carrera: int):
        """Muestra formulario para editar carrera existente usando función unificada"""
        try:
            # Obtener datos de la carrera
            carreras = obtener_listado_carreras()
            carrera = next((c for c in carreras if c['id_carrera'] == id_carrera), None)
            ...
            with st.form(f"form_editar_carrera_{id_carrera}"):
                ...
                if save_button:
                    # Usar función unificada para editar carrera
                    activo = carrera.get('activo', True)
                    if editar_carrera(id_carrera, nombre_editar, codigo_editar, activo):
                        st.success("✅ Carrera actualizada exitosamente")
                        st.rerun()
                    else:
                        st.error("❌ Error al actualizar la carrera")
    ...
    def mostrar_estadisticas_carreras(self):
        ...
        try:
            ...
            with col1:
                if resultado_total:
                    if isinstance(resultado_total, list) and len(resultado_total) > 0:
                        st.metric("Total Carreras", resultado_total[0].get('total', 0))
                    elif isinstance(resultado_total, dict):
                        st.metric("Total Carreras", resultado_total.get('total', 0))
                    else:
                        st.metric("Total Carreras", resultado_total)
            
            with col2:
                if resultado_activas:
                    if isinstance(resultado_activas, list) and len(resultado_activas) > 0:
                        st.metric("Carreras Activas", resultado_activas[0].get('activas', 0))
                    elif isinstance(resultado_activas, dict):
                        st.metric("Carreras Activas", resultado_activas.get('activas', 0))
                    else:
                        st.metric("Carreras Activas", resultado_activas)
            
            # Separador
            st.divider()
            
            # Botones de acción para cada carrera
            for carrera in carreras:
                col_edit, col_delete = st.columns([1, 1])
                
                with col_edit:
                    if st.button(f"✏️ Editar", key=f"edit_{carrera['id_carrera']}"):
                        self.mostrar_formulario_edicion(carrera['id_carrera'])
                
                with col_delete:
                    if st.button(f"🗑️ Eliminar", key=f"delete_{carrera['id_carrera']}"):
                        self.eliminar_carrera(carrera['id_carrera'])
                        
        except Exception as e:
            st.error(f"Error mostrando carreras: {e}")

    def obtener_carrera_por_id(self, id_carrera: int):
