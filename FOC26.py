...
from seguridad import SeguridadFOC26, admin_required, profesor_required, estudiante_required, login_required
...
class DatabaseFOC26:
    ...
    def __init__(self):
        ...
        if info['source'] == 'railway':
            print(f"🚀 Database FOC26 inicializado para Railway")
        else:
            print(f"🏠 Database FOC26 inicializado para Local")
    ...
    def desconectar(self):
        ...
        try:
            ...
            print(f"Error desconectando: {e}") FOC26DB: {e}")
    ...
    try:
        ...
        with tab1:
            ...
            if SeguridadFOC26.is_admin():
                # Admin ve todos los profesores sin filtros
                query = """
                ...
                query = f"""
                    ...
                    JOIN persona p ON pr.id_persona = p.id
                    JOIN usuario u ON pr.id_usuario = u.id
                    WHERE pr.cedula_profesor = '{user_cedula}'
                    ORDER BY p.apellido, p.nombre
                """
            else:
            ...
            if profesores is None or len(profesores) == 0:
                ...
                # Dataframe con botones de acción según rol
                if SeguridadFOC26.is_admin():
                    # Admin ve tabla con botones de eliminar - CORREGIDO: ButtonColumn no existe
                    df_profesores['acciones'] = 'Eliminar'
                    st.dataframe(
                        df_profesores[[
                            'cedula_profesor', 'nombre', 'apellido', 'especialidad',
                            'departamento', 'correo_personal', 'estado', 'acciones'
                        ]],
                        use_container_width=True,
                        column_config={
                            "acciones": st.column_config.TextColumn("Acciones")
    ...
    try:
        ...
        if not st.session_state.get('autenticado', False):
            ...
            with st.sidebar:
                ...
                if str(st.session_state.get('user', {}).get('rol', '')).strip().upper() in ['ADMIN', 'ADMINISTRADOR']:
                    opciones_menu.extend(['Estudiantes', 'Profesores', 'Formacion Complementaria', 'Notas', 'Reportes'])
                elif SeguridadFOC26.is_profesor():
