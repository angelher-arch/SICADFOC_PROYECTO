...
def asegurar_estructura_bd():
    ...
    try:
        # Iniciando autoreparación de base de datos
        
        # 1. Importar y verificar configuración
        from database import DatabaseManager
        db_manager = DatabaseManager()
        
        # Base de datos detectada y configurada
        ...
        try:
            ...
            cursor.close()
            # Tabla 'usuarios' ya existe - sin reparación necesaria
            conn.close()
            return True
        except Exception as e:
            # Diagnóstico: Tabla 'usuarios' no existe
            cursor.close()
...
try:
    ...
    st.error(f"Error importando módulos principales: {e}")
    sys.exit(1)
    ...
    if not st.session_state.logged_in:
        ...
        if 'modulo_actual' in st.session_state:
            ...
            if tiene_acceso:
                ...
                elif st.session_state.modulo_actual == "Configuración":
                    configuracion_main()
            else:
            ...
            if tiene_permiso(st.session_state.user_role, 'Configuración', 'acceso'):
                modulos_disponibles.append("Configuración")
