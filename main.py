...
# Cargar variables de entorno ANTES de importar el servicio unificado
...
def asegurar_estructura_bd():
    ...
    try:
        ...
        from servicio_unificado_optimizado import obtener_servicio, ejecutar_query
...
if 'modulo_anterior' in st.session_state and st.session_state.modulo_anterior != st.session_state.get('modulo_actual', ''):
    ...
    try:
        from servicio_unificado_optimizado import limpiar_conexiones
        limpiar_conexiones()
...
# Importar servicio_unificado_optimizado como Single Source of Truth
from servicio_unificado_optimizado import obtener_servicio, ejecutar_query, test_conexion, limpiar_conexiones
...
# LIMPIEZA DE CONEXIONES AL INICIO - PURGAR SESIONES INACTIVAS
print("=== LIMPIEZA DE CONEXIONES AL INICIO ===")
try:
    limpiar_conexiones()
    print("OK Conexiones limpiadas al inicio")
except Exception as e:
    print(f"Error limpiando conexiones: {e}")
...
# Importaciones del servicio unificado optimizado (único punto de acceso)
from servicio_unificado_optimizado import obtener_servicio, test_conexion, es_produccion, es_local, limpiar_conexiones

# Importaciones de módulos principales (usando servicio unificado)
from gestion_estudiantil import gestion_estudiantil, registro_estudiantes_main
from gestion_profesores import gestion_profesores_main, registro_profesores_main
            ...
            if not tiene_acceso:
                try:
                    ...
                    if db_info.get('status') != 'connected':
                        st.error("Error de conexión a la base de datos en la nube (Render)")
                        st.markdown("#### Configuración Cloud")
                        st.write("El sistema utiliza exclusivamente FOC26DBCloud (Render). Verifique:")
                        st.write("- 🌐 Conexión a Render Cloud activa")
                        st.write("- 🔒 SSL Mode: Require")
                        st.write("- 📡 Variables de entorno configuradas")
                        st.write("🔄 La conexión se reestablecerá automáticamente en el próximo intento.")
                except Exception as e:
