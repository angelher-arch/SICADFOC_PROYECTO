...
# Configurar logging para health check
...
# Conexion a base de datos FOC26DB - CLOUD-READY con health check
def conectar_foc26db():
    """Conectar a la base de datos FOC26DB con health check"""
    global db_connected, db_error, db
    
    try:
        # Importar gestor de base de datos centralizado
        from database_manager import get_database_manager
        from database_config import get_database_config
        
        # Obtener configuración y mostrar health check
        config = get_database_config()
        
        # Health check en logs
        logger.info("=== HEALTH CHECK BASE DE DATOS ===")
        logger.info(f"Entorno: {'Cloud/Producción' if config.is_production() else 'Local/Desarrollo'}")
        logger.info(f"Host: {config.db_config.get('host', 'desconocido')}")
        logger.info(f"Base de datos: {config.db_config.get('dbname', 'desconocida')}")
        logger.info(f"Usuario: {config.db_config.get('user', 'desconocido')}")
        logger.info("Contraseña: [OCULTA]")
        logger.info("=== FIN HEALTH CHECK ===")
        ...
        if db_manager.is_connected:
            ...
            if db_manager.health_check():
                ...
                db_error = None
                db = db_manager
                logger.info("Health check exitoso - Base de datos conectada")
                return True
            else:
                db_connected = False
                db_error = "Health check falló - Conexión no funcional"
                logger.error("Health check falló")
                return False
        else:
            db_connected = False
            db_error = "No se pudo establecer conexión con FOC26DB"
        ...
        db_connected = False
        db_error = str(e)
        logger.error(f"Error en conectar_foc26db: {e}")
        return False

def validar_login_directo(cedula, password):
    """Función de login definitiva usando exclusivamente cédula como identificador"""
    try:
        from database_manager import get_database_manager
        from seguridad import verify_password, is_sha256_hash
        
        # Normalizar cédula (quitar espacios y mantener formato)
        cedula_normalizada = str(cedula).strip()
        
        logger.info(f"Intento de login con cédula: {cedula_normalizada}")
        
        # Obtener gestor de base de datos
        db_manager = get_database_manager()
        
        if not db_manager.is_connected:
            logger.error("Base de datos no conectada para login")
            return {'success': False, 'message': 'Error de conexión a la base de datos'}
        
        # Consultar usuario por CÉDULA (identificador único)
        query = """
        SELECT 
            u.id,
            u.cedula_usuario,
        ...
        WHERE u.cedula_usuario = %s AND u.activo = true
        """
        
        resultado = db_manager.execute_query(query, (cedula_normalizada,))
        
        if not resultado or len(resultado) == 0:
            logger.warning(f"Usuario no encontrado con cédula: {cedula_normalizada}")
            return {'success': False, 'message': 'Cédula no registrada o usuario inactivo'}
        
        usuario = resultado[0]
        
        # Verificar contraseña
        if verify_password(usuario['contrasena'], password):
            # Login exitoso - retornar datos del usuario
            logger.info(f"Login exitoso para cédula: {cedula_normalizada}")
            ...
            return {
                ...
                'usuario': {
                    ...
                    'login_usuario': usuario['cedula_usuario'],  # Login = cédula
                    'email': usuario['email'],
                    'rol': usuario['rol'],
            ...
            logger.warning(f"Contraseña incorrecta para cédula: {cedula_normalizada}")
            return {'success': False, 'message': 'Cédula o contraseña incorrectos'}
            
    except Exception as e:
        logger.error(f"Error en validar_login_directo: {e}")
            ...
            if submit_button:
                if cedula_input and password:
                    ...
                    if resultado_login['success']:
                        ...
                        st.success(f"Bienvenido: {resultado_login['usuario']['nombre']} {resultado_login['usuario']['apellido']}")
                        st.rerun()
                    else:
    ...
    try:
        ...
        if not st.session_state.get('autenticado', False):
            ...
            with st.sidebar:
                ...
                # DEBUG: Logs temporales para depuración de navegación
                rol_bruto = str(user.get('rol', '')).strip()
                rol_normalizado = rol_bruto.strip().capitalize()
                rol_upper = rol_bruto.strip().upper()
                
                st.info(f"Rol: {user['rol']}")
                # Logs detallados (ocultos en producción)
                if os.getenv('ENVIRONMENT', '').lower() in ['local', 'development']:
                    with st.expander("🔍 DEBUG - Roles y Navegación", expanded=False):
                        st.write(f"**Rol bruto:** `{rol_bruto}`")
                        st.write(f"**Rol normalizado:** `{rol_normalizado}`")
                        st.write(f"**Rol upper:** `{rol_upper}`")
                        
                        # Evaluar condiciones
                        from seguridad import SeguridadFOC26
                        st.write(f"**is_admin():** {SeguridadFOC26.is_admin()}")
                        st.write(f"**is_profesor():** {SeguridadFOC26.is_profesor()}")
                        st.write(f"**is_estudiante():** {SeguridadFOC26.is_estudiante()}")
                        
                        # Condición del menú
                        condicion_menu = rol_upper in ['ADMIN', 'ADMINISTRADOR', 'ADMIN']
                        st.write(f"**Condición menú Admin:** {condicion_menu}")
                        
                        if condicion_menu:
                            st.success("✅ Admin debería ver todos los módulos")
                        else:
                            st.error("❌ Admin NO cumple la condición del menú")
                ...
                # CORRECCIÓN: Incluir 'Admin' con mayúscula/minúscula para consistencia con BD
                if str(st.session_state.get('user', {}).get('rol', '')).strip().upper() in ['ADMIN', 'ADMINISTRADOR', 'ADMIN']:
