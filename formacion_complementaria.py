...
def modulo_formacion_complementaria(db):
    ...
    try:
        ...
        with tab1:
            st.subheader("🏗️ Crear Nuevo Taller")
            ...
            with st.form("form_crear_taller"):
                ...
                with col1:
                    ...
                    tipo_taller = st.selectbox("Tipo de Taller*", get_tipos_taller())
                    ...
                    nivel_academico = st.selectbox("Nivel Académico*", get_niveles_academicos())
                ...
                with col2:
                    ...
                    if profesores_disponibles:
                        ...
                        profesor_seleccionado = st.selectbox("Instructor*", opciones_profesores)
        ...
        with tab2:
            ...
            # Construir consulta dinámica usando formación y taller
            query = """
        ...
        with tab3:
            ...
            with col1:
                st.write("📝 **Inscribir Estudiante en Taller:**")
                
                # Seleccionar estudiante
                estudiantes = db.ejecutar_consulta("""
                    SELECT u.id_usuario, p.nombre, p.apellido, e.codigo_estudiantil
                    FROM estudiante e
                ...
                if estudiantes is not None and len(estudiantes) > 0:
                    opciones_estudiantes = [f"{row['apellido']}, {row['nombre']} - {row['codigo_estudiantil']}" for _, row in estudiantes.iterrows()]
                    estudiante_seleccionado = st.selectbox("Seleccionar Estudiante:", opciones_estudiantes)
                    
                    estudiante_datos = estudiantes[estudiantes['codigo_estudiantil'] == estudiante_seleccionado.split(' - ')[-1]].iloc[0]
                    ...
                    if formaciones is not None and len(formaciones) > 0:
                        opciones_formaciones = [f"{row['nombre_cohorte']} - {row['nombre_taller']} (Disponibles: {row['cupos_disponibles']})" for _, row in formaciones.iterrows()]
                        formacion_seleccionada = st.selectbox("Seleccionar Formación/Taller:", opciones_formaciones)
                        
                        formacion_datos = formaciones[formaciones['nombre_cohorte'] == formacion_seleccionada.split(' - ')[0]].iloc[0]
            ...
            with col2:
                ...
                if formaciones_ins is not None and len(formaciones_ins) > 0:
                    opciones_formaciones_ins += [f"{row['nombre_cohorte']} - {row['nombre_taller']}" for _, row in formaciones_ins.iterrows()]

                filtro_taller = st.selectbox("Filtrar por Formación:", opciones_formaciones_ins)
                
                if st.button("🔍 Consultar Inscripciones"):
                    query_inscripciones = """
                        SELECT i.id_inscripcion, i.fecha_inscripcion, i.estado,
                               p.nombre, p.apellido, e.codigo_estudiantil,
