...
# IMPORTACIONES LOCALES AL MÓDULO - MOTOR CENTRAL UNIFICADO CON POOL
try:
    from seguridad import tiene_permiso, SeguridadFOC26
    from database import motor_central
    from servicio_unificado_optimizado import obtener_servicio, limpiar_conexiones
...
class MotorFormacionComplementaria:
    ...
    def crear_formacion(self, datos):
        """Crear nueva formación complementaria con cierre de conexión garantizado"""
        try:
            resultado = self.motor.operacion_crud_unificada('formacion_complementaria', 'CREATE', datos)
            return resultado
        finally:
    ...
    def leer_formaciones(self, filtros=None, orden='fecha_inicio DESC'):
        """Leer formaciones complementarias con cierre de conexión garantizado"""
        try:
            resultado = self.motor.operacion_crud_unificada('formacion_complementaria', 'READ', filtros=filtros, orden=orden)
            return resultado
        finally:
            # Forzar cierre de conexión
            try:
    ...
    def actualizar_formacion(self, datos, filtros):
        """Actualizar formación complementaria con cierre de conexión garantizado"""
        try:
            resultado = self.motor.operacion_crud_unificada('formacion_complementaria', 'UPDATE', datos, filtros)
            return resultado
        finally:
            # Forzar cierre de conexión
            try:
    ...
    def eliminar_formacion(self, filtros):
        """Eliminar formación complementaria con cierre de conexión garantizado"""
        try:
            resultado = self.motor.operacion_crud_unificada('formacion_complementaria', 'DELETE', filtros=filtros)
            return resultado
        finally:
            # Forzar cierre de conexión
            try:
    ...
    def crear_estudiante(self, datos):
        """Crear nuevo estudiante con cierre de conexión garantizado"""
        try:
            resultado = self.motor.operacion_crud_unificada('estudiante', 'CREATE', datos)
            return resultado
        finally:
            # Forzar cierre de conexión
            try:
    ...
    def leer_estudiantes(self, filtros=None, orden='apellido, nombre'):
        """Leer estudiantes con información de persona y cierre de conexión garantizado"""
        try:
            ...
            if filtros:
                ...
                if condiciones:
                    ...
                    resultado = self.motor.ejecutar_consulta_personalizada(query, tuple(valores))
                else:
    ...
    def actualizar_estudiante(self, datos, filtros):
        """Actualizar estudiante con cierre de conexión garantizado"""
        try:
            resultado = self.motor.operacion_crud_unificada('estudiante', 'UPDATE', datos, filtros)
            return resultado
        finally:
            # Forzar cierre de conexión
            try:
    ...
    def eliminar_estudiante(self, filtros):
        """Eliminar estudiante con cierre de conexión garantizado"""
        try:
            resultado = self.motor.operacion_crud_unificada('estudiante', 'DELETE', filtros)
            return resultado
        finally:
            # Forzar cierre de conexión
            try:
    ...
    def crear_profesor(self, datos):
        """Crear nuevo profesor con cierre de conexión garantizado"""
        try:
            resultado = self.motor.operacion_crud_unificada('profesor', 'CREATE', datos)
            return resultado
        finally:
            # Forzar cierre de conexión
            try:
    ...
    def leer_profesores(self, filtros=None, orden='apellido, nombre'):
        """Leer profesores con información de persona y cierre de conexión garantizado"""
        try:
    ...
    def actualizar_profesor(self, datos, filtros):
        """Actualizar profesor con cierre de conexión garantizado"""
        try:
            resultado = self.motor.operacion_crud_unificada('profesor', 'UPDATE', datos, filtros)
            return resultado
        finally:
            # Forzar cierre de conexión
            try:
    ...
    def eliminar_profesor(self, filtros):
        """Eliminar profesor con cierre de conexión garantizado"""
        try:
            resultado = self.motor.operacion_crud_unificada('profesor', 'DELETE', filtros)
            return resultado
        finally:
            # Forzar cierre de conexión
            try:
...
def listar_y_editar_talleres(rol_usuario):
    ...
    try:
        # USAR MOTOR CENTRAL UNIFICADO
        resultado = motor_formacion.leer_formaciones(orden='fecha_inicio DESC')
...
def talleres_disponibles(db, rol_usuario):
    ...
    try:
        ...
        columnas_disponibles = ['nombre_taller', 'fecha_inicio', 'fecha_fin', 'cupo_disponible']
        columnas_finales = []
