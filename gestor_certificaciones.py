    ...
    def guardar_certificado_db(self, certificado_data: Dict[str, Any]):
        ...
        try:
            ...
            if trans_module:
                ...
                # Implementación directa usando arquitectura centralizada
    ...
    def obtener_certificados_internos(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if db:
                ...
                """
                cursor = db.cursor()
                cursor.execute(query, (self.user_cedula,))
    ...
    def validar_certificado(self, cert_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if db:
                ...
                cursor.close()
                if result:
                    return {'success': True, 'certificado': dict(result), 'tipo': 'interno'}
                ...
                cursor.close()
                if result:
                    return {'success': True, 'certificado': dict(result), 'tipo': 'externo'}
...
def gestor_certificaciones_unificado():
    ...
    try:
        ...
        # Vista única sin tabs
        st.markdown("### Certificados Internos")
        interfaz_certificados_internos(gestor)
        
        st.markdown("---")
        st.markdown("### Certificados Externos")
...
def gestor_certificaciones_unificado():
    """Función principal unificada del gestor de certificaciones"""
    try:
        gestor = GestorCertificaciones()
        ...
        if not tiene_permiso(rol_usuario, 'Certificados', 'Consultar'):
            st.error("Acceso denegado. No tienes permisos para consultar certificados.")
            return
        
        st.header("🏆 Gestión de Certificados")
        
        # Tabs para diferentes funcionalidades
        tab1, tab2, tab3 = st.tabs(["📋 Listado", "🎨 Diseñador", "👁️ Previsualizar"])
        
        with tab1:
            gestor.mostrar_listado_certificados()
        
        with tab2:
            # Solo administradores pueden acceder al diseñador
            if tiene_permiso(rol_usuario, 'Certificados', 'Editar'):
                from certificado_designer import certificado_designer_main
                certificado_designer_main()
            else:
                st.warning("El diseñador de certificados solo está disponible para administradores.")
        
        with tab3:
            # Previsualización disponible para roles autorizados
            if tiene_permiso(rol_usuario, 'Certificados', 'Consultar'):
                from certificado_designer import CertificadoDesigner
                designer = CertificadoDesigner()
                designer.previsualizar_certificado()
            else:
                st.warning("No tienes permisos para previsualizar certificados.")
                
    except Exception as e:
        st.error(f"Error en el módulo de certificaciones: {e}")
